...
export type AuthorityCreateAbortedError =
...
export class AuthorityCreateAborted extends reboot_api.Aborted {
  static fromStatus(status: reboot_api.Status) {
    ...
    if (error !== undefined) {
      return new AuthorityCreateAborted(
...
export type AuthorityApplyAbortedError =
  reboot_api.InstanceTypeForErrorTypes<
...
export class AuthorityApplyAborted extends reboot_api.Aborted {
  static fromStatus(status: reboot_api.Status) {
    ...
    if (error !== undefined) {
      return new AuthorityApplyAborted(
        error, { message: status.message }
      );
    }
...
export type AuthorityChangesAbortedError =
  reboot_api.InstanceTypeForErrorTypes<
...
export class AuthorityChangesAborted extends reboot_api.Aborted {
  static fromStatus(status: reboot_api.Status) {
    ...
    if (error !== undefined) {
      return new AuthorityChangesAborted(
        error, { message: status.message }
      );
    }
...
class AuthorityInstance {
      ...
      await fetch(
        new Request(
          `${this.endpoint}/__/reboot/rpc/${this.stateRef}/rbt.v1alpha1.React/WebSocketsConnection`,
          {
            method: "POST",
            headers,
            body: new reboot_api.react_pb.WebSocketsConnectionRequest()
              .toJsonString(),
          }
        ),
      ...
      async (resolve, reject) => {
        ...
        switch (responseOrStatus.case) {
          ...
          case "status": {
            ...
            console.warn(
              `'Authority.Create' aborted with ${aborted.message}`
            );
  ...
  >> {
    ...
    this.useApplyMutations = this.useApplyMutations.concat(mutation);
    ...
    return new Promise<
      ...
      async (resolve, reject) => {
        ...
        const removeMutationsAndSetPending = () => {
          this.useApplyMutations =
            this.useApplyMutations.filter(m => m !== mutation);
        ...
        switch (responseOrStatus.case) {
          case "response": {
            await observed(() => {
              removeMutationsAndSetPending();
              resolve({
                response: ApplyResponse.fromBinary(
                  responseOrStatus.value
                )
              });
            });
          ...
          case "status": {
            ...
            console.warn(
              `'Authority.Apply' aborted with ${aborted.message}`
            );
...
export const useAuthority = (
  ...
  const [instance, setInstance] = useState(() => {
    return AuthorityInstance.use(
      stateId, stateRef, endpoint
    );
  });

  if (instance.id !== stateId) {
    setInstance(
      AuthorityInstance.use(
        stateId, stateRef, endpoint
      )
    );
  }
  ...
  function useCreate() {
    const [
      pending,
      setPending
    ] = useState<reboot_react.Mutation<CreateRequest>[]>([]);
    ...
    const create = useMemo(() => {
      ...
      method.pending =
        new Array<reboot_react.Mutation<CreateRequest>>();
  ...
  function useApply() {
    const [
      pending,
      setPending
    ] = useState<reboot_react.Mutation<ApplyRequest>[]>([]);
    ...
    const apply = useMemo(() => {
      ...
      method.pending =
        new Array<reboot_react.Mutation<ApplyRequest>>();
  ...
  function useChanges(
    ...
    const newRequest = partialRequest instanceof ChangesRequest
      ? partialRequest.clone()
    ...
    useEffect(() => {
      ...
      instance.useChanges(
        ...
        (status: reboot_api.Status) => {
          ...
          console.warn(
            `'Authority.Changes' aborted with ${aborted.message}`
          );
  ...
  async function changes(
    ...
    const { response, aborted } = await (async () => {
      ...
      while (true) {
        ...
        console.warn(`Retrying call to rbt.thirdparty.prosemirror.v1.AuthorityMethods.Changes after backoff ...`);
