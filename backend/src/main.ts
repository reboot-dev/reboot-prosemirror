...
export class AuthorityServicer extends Authority.Servicer {
  ...
  private doc(stateId: string) {
    ...
    if (version < this.state.changes.length) {
      const steps = this.state.changes
        .slice(version)
  ...
  async create(
    ...
    return {
      doc: Struct.fromJson(this.doc(context.stateId).toJSON()),
      version: this.state.changes.length,
    };
  }
  ...
  async apply(
    ...
    if (request.version != this.state.changes.length) {
    ...
    let doc = this.doc(context.stateId);
  ...
  async changes(
    ...
    if (sinceVersion > this.state.changes.length) {
      throw new Authority.ChangesAborted(new errors_pb.InvalidArgument());
    }

    return {
      version: sinceVersion,
      changes: this.state.changes.slice(sinceVersion),
    };
  }
}
